...
public class DetailHoaDonDialog extends javax.swing.JDialog {
    ...
    private void fillInput() {
        if (listCTHD == null || listCTHD.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Chi tiết hóa đơn trống hoặc không tìm thấy!", "Thông báo", JOptionPane.WARNING_MESSAGE);
            return;
        }

        HoaDon hoaDon = listCTHD.get(0).getHoaDon();
        if (hoaDon != null) {
            txtMaHD.setText(hoaDon.getId() != null ? hoaDon.getId() : "");

            if (hoaDon.getKhachHang() != null) {
                txtTenKH.setText(hoaDon.getKhachHang().getHoTen() != null ? hoaDon.getKhachHang().getHoTen() : "");
            } else {
                txtTenKH.setText("");
            }

            if (hoaDon.getNhanVien() != null) {
                txtTenNV.setText(hoaDon.getNhanVien().getHoTen() != null ? hoaDon.getNhanVien().getHoTen() : "");
            } else {
                txtTenNV.setText("");
            }
        }
